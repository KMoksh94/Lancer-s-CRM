...
const AddProjectModal = ({setOpenAddModal}) => {
  ...
  <div className="w-xl bg-white rounded-2xl shadow-xl p-5 space-y-4">
    ...
    <div>
      ...
      <div className="flex gap-3">
        <button className={`px-4 py-2 rounded-lg border hover:bg-indigo-100  ${activeStatus === 'Active' ? "bg-indigo-100 text-indigo-700" : 'bg-white'} hover:bg-indigo-100 hover:text-indigo-700`}
        onClick={()=>{setActiveStatus('Active')
          setNewProjectData(newProjectData=>({...newProjectData,status : 'Active', paymentDate : ''}))
        }}>
          Active
        </button>
        <button className={`px-4 py-2 rounded-lg border ${activeStatus === 'Paid' ? "bg-green-100 text-green-700" : 'bg-white'} hover:bg-green-100 hover:text-green-700`}
        onClick={()=>{setActiveStatus('Paid')
          setNewProjectData(newProjectData=>({...newProjectData,status : 'Paid', paymentDate : new Date().toISOString()}))
        }}>
          Paid
        </button>
        <button className={`px-4 py-2 rounded-lg border ${activeStatus === 'Overdue' ? "bg-red-100 text-red-700" : 'bg-white'} hover:bg-red-100 hover:text-red-700`}
        onClick={()=>{setActiveStatus('Overdue')
          setNewProjectData(newProjectData=>({...newProjectData,status : 'Overdue', paymentDate : ''}))
        }}>
          Overdue
        </button>
      </div>
